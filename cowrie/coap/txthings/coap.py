...
log = Logger(observer=jsonFileLogObserver(io.open("log/coap.json", "a")),
             namespace="saver")
...
class Coap(protocol.DatagramProtocol):
    ...
    def datagramReceived(self, data, (host, port)):
        # log.info("Received %r from %s:%d" % (data, host, port))
        log.info(eventid='cowrie.coap.received',
                protocol='CoAP',
                data=data,
                host=host,
                port=port)
    ...
    def deduplicateMessage(self, message):
        ...
        # log.info("Incoming Message ID: %d" % message.mid)
        log.info(eventid='cowrie.coap.message',
                protocol='CoAP',
                data="Incoming Message ID: %d" % message.mid,
                host="",
                port="")
        if message.mtype in (CON, NON):
            if key in self.recent_remote_ids:
                if message.mtype is CON:
                    if len(self.recent_remote_ids[key]) == 3:
                        # log.info('Duplicate CON received, sending old response again')
                        log.info(eventid='cowrie.coap.message',
                                protocol='CoAP',
                                data='Duplicate CON received, sending old response again',
                                host="",
                                port="")
                        self.sendMessage(self.recent_remote_ids[key][2])
                    else:
                        # log.info('Duplicate CON received, no response to send')
                        log.info(eventid='cowrie.coap.message',
                                protocol='CoAP',
                                data='Duplicate CON received, no response to send',
                                host="",
                                port="")
                else:
                    # log.info('Duplicate NON received')
                    log.info(eventid='cowrie.coap.message',
                            protocol='CoAP',
                            data='Duplicate NON received',
                            host="",
                            port="")
                return True
            else:
                # log.info('New unique CON or NON message received')
                log.info(eventid='cowrie.coap.message',
                        protocol='CoAP',
                        data='New unique CON or NON message received',
                        host="",
                        port="")
            ...
            if key in self.recent_local_ids:
                # log.info('Duplicate ACK or RST received')
                log.info(eventid='cowrie.coap.message',
                        protocol='CoAP',
                        data='Duplicate ACK or RST received',
                        host="",
                        port="")
                return True
            else:
                # log.info('New unique ACK or RST message received')
                log.info(eventid='cowrie.coap.message',
                        protocol='CoAP',
                        data='New unique ACK or RST message received',
                        host="",
                        port="")
    ...
    def processResponse(self, response):
        ...
        def resetUnrecognized():
            log.info("Response not recognized - sending RST.")
        ...
        if response.mtype is ACK:
            if response.mid in self.active_exchanges:
                ...
                return
        log.info("Received Response, token: %s, host: %s, port: %s" % (response.token.encode('hex'), response.remote[0], response.remote[1]))
    ...
    def processRequest(self, request):
        ...
        if (uriPathAsString(request.opt.uri_path), request.remote) in self.incoming_requests:
            log.info("Request pertains to earlier blockwise requests.")
    ...
    def processEmpty(self, message):
        if message.mtype is CON:
            log.info('Empty CON message received (CoAP Ping) - replying with RST.')
    ...
    def sendMessage(self, message):
        ...
        host = str(address)
        target = (host, port)
        log.info("Sending message to %s:%d" % (host, port))
        ...
        log.info("Message %r sent successfully" % msg)
    ...
    def addExchange(self, message):
        ...
        log.info("Exchange added, Message ID: %d." % message.mid)
    ...
    def removeExchange(self, message):
        ...
        log.info("Exchange removed, Message ID: %d." % message.mid)
    ...
    def retransmit(self, message, timeout, retransmission_counter):
        ...
        if retransmission_counter < MAX_RETRANSMIT:
            ...
            next_retransmission = reactor.callLater(timeout, self.retransmit, message, timeout, retransmission_counter)
            self.active_exchanges[message.mid] = (message, next_retransmission)
            log.info("Retransmission, Message ID: %d." % message.mid)
        else:
            pass
...
class Requester(object):
    ...
    def sendRequest(self, request):
        ...
        try:
            ...
            log.info("Sending request - Token: %s, Host: %s, Port: %s" % (request.token.encode('hex'), str(request.remote[0]), request.remote[1]))
    ...
    def processBlock1InResponse(self, response):
        ...
        if response.opt.block1 is not None:
            ...
            log.info("Response with Block1 option received, number = %d, more = %d, size_exp = %d." % (block1.block_number, block1.more, block1.size_exponent))
    ...
    def sendNextRequestBlock(self, result, next_block):
        ...
        log.info("Sending next block of blockwise request.")
    ...
    def processBlock2InResponse(self, response):
        ...
        if response.opt.block2 is not None:
            block2 = response.opt.block2
            log.info("Response with Block2 option received, number = %d, more = %d, size_exp = %d." % (block2.block_number, block2.more, block2.size_exponent))
            if self.assembled_response is not None:
                ...
                if block2.block_number is 0:
                    log.info("Receiving blockwise response")
                    self.assembled_response = response
                else:
                    log.info("ProcessBlock2 error: transfer started with nonzero block number.")
                    return defer.fail()
    ...
    def askForNextResponseBlock(self, result, request):
        ...
        log.info("Requesting next block of blockwise response.")
        self.deferred = self.sendRequest(request)
        self.deferred.addCallback(self.processBlock2InResponse)
        return self.deferred
...
class Responder(object):
    ...
    def __init__(self, protocol, request):
        ...
        log.info("Request doesn't pertain to earlier blockwise requests.")
    ...
    def processBlock1InRequest(self, request):
        ...
        if request.opt.block1 is not None:
            ...
            log.info("Request with Block1 option received, number = %d, more = %d, size_exp = %d." % (block1.block_number, block1.more, block1.size_exponent))
            if block1.block_number == 0:
                ...
                log.info("New or restarted incoming blockwise request.")
            ...
            if block1.more is True:
                ...
                log.info("Complete blockwise request received.")
                return defer.succeed(self.assembled_request)
        else:
            if self.assembled_request is not None:
                log.info("Non-blockwise request received during blockwise transfer. Blockwise transfer cancelled.")
    ...
    def acknowledgeRequestBlock(self, request):
        ...
        log.info("Sending block acknowledgement (allowing client to send next block).")
    ...
    def processBlock2InRequest(self, request):
        ...
        if request.opt.block2 is not None:
            ...
            log.info("Request with Block2 option received, number = %d, more = %d, size_exp = %d." % (block2.block_number, block2.more, block2.size_exponent))
            ...
            if (2 ** (block2.size_exponent + 4)) * block2.block_number == sent_length:
                ...
                log.info("Incorrect block number requested")
                return defer.fail()
        else:
            return defer.fail()
    ...
    def sendResponseBlock(self, response_block, request):
        ...
        log.info("Sending response block.")
    ...
    def sendNonFinalResponse(self, response, request):
        ...
        def cancelNonFinalResponse(d):
            log.info("Waiting for next client request cancelled")
        ...
        def timeoutNonFinalResponse(d):
            ...
            log.info("Waiting for next blockwise request timed out")
            self.protocol.incoming_requests.pop((uriPathAsString(request.opt.uri_path), request.remote))
    ...
    def sendResponse(self, response, request):
        ...
        log.info("Token: %s" % ":".join("{:02x}".format(ord(c)) for c in response.token))
        ...
        log.info("Sending response, type = %s (request type = %s)" % (types[response.mtype], types[request.mtype]))
    ...
    def sendEmptyAck(self, request):
        ...
        log.info("Response preparation takes too long - sending empty ACK.")
...
class Observation(object):
    ...
    def trigger(self):
        ...
        log.info("Triggering retransmission with original request %r (will set response_type to ACK)" % vars(self.original_request))
